...
const loadTicker = async (symbol) => {
  try {
    ...
    if (tickerDetails) {
      const data = _.pick(tickerDetails, [
        'name',
        'symbol',
        'logo',
        'country',
...
exports.newTrade = async (symbol) => {
  redis.get(symbol, async (error, price) => {
    try {
      if (!price) {
        return;
      }
      ...
      strategies.map(async (strategy) => {
        ...
        buySignals = !position
          ? buySignals.reduce((string, signal) => (string
            += eval(
              percentageDifference(
        ...
        sellSignals = position
          ? sellSignals.reduce((string, signal) => (string
            += eval(
              percentageDifference(
                eval(signal.operand_1),
                eval(signal.operand_2)
...
exports.backtest = async (strategyId, symbol, years, startValue) => {
  try {
    ...
    const buySignals = strategy.signals.filter((signal) => signal.type === 'buy');
    const sellSignals = strategy.signals.filter((signal) => signal.type === 'sell');
    ...
    const result = await aggregates.reduce(
      async (status, agg) => {
        ...
        const buyQuantity = (status.account[strategy.buy_percentage_type_of]
          * strategy.buy_percentage)
          / price;
    ...
    const trades = result.trades.map((trade, i) => {
      if (trade.value < 0) {
        trade.profit = (trade.price - result.trades[i - 1].price)
          * trade.quantity
          * -1;
      }
